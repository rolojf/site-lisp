...
;;; Commentary:
;;; Code:
(defun rolo-procesa-think ()
  "Replace <think> with #+begin_think and </think> with #+end_think"
  (interactive)
  (save-excursion
    (goto-char (point-min))
    (replace-string "<think>" "#+begin_think" nil nil)
    (goto-char (point-min))
    (replace-string "</think>" "#+end_think" nil nil)
